...
var artists = function(page, artist, details, callback) {
  db.all("SELECT Artist.ArtistId, Name, StarsNo " +
    ...
    "ORDER BY Name LIMIT "+ itemsPerPage + " OFFSET ($page - 1) * "+itemsPerPage, /* vrni mi 33 zapisov in preskoci prvih...0, 33, 66...listamo po straneh*/ /*NALOGA 2*/
    {$page: page}, function(error, rows) {
      if (error) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          result += '<div id="' + rows[i].ArtistId + '"><span class="numbers">' + (page * itemsPerPage + i - (itemsPerPage-1)) + '.</span>' + /* naloga 2 */
...
app.get('/artists/:page', function(request, response) {
  artists(request.params.page, -1, '', function(result) { /* vrne seznam artistov za podano-prvo stran...pričakujemo seznam vseh artistov*/
    response.render('index', {content: result}); /* prikazemo uporabniku...poisce index.ejs in uporabi...tam se spreminja samo content ki se priredi resultu za artiste*/
  });
});
...
app.get('/pages', function(request, response) {
  db.get("SELECT COUNT(*) AS Artists FROM Artist", function(error, row) {
    if (error) {
      ...
      response.send({pages: Math.ceil(row.Artists / itemsPerPage)}); /* naloga 2 */
  });
});

/* responds with first page's artists */  /* naloga 1*/
